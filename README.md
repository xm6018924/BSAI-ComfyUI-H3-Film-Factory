...
---
## 2026-09-04 修复：单独选择 CLIP 前置缺失时自动补渲染（不再报错）
> **v1.17 新增**

### 问题
- v1.15 在磁盘 latent 链缺失时直接报错（`前置 latent 链缺失，请先全量渲染`）——用户单选 CLIP 无法直接生成，体验差。

### v1.17 修复
- **缓存完整时**：单独选择 CLIPn 严格从 CLIPn 开始连续渲染到结束（不补前置）。
- **缓存缺失时**：自动补渲染缺失的前置段建立完整链（H3 必须依赖前置 latent），并打印明确提示：
  `前置 latent 链缺失（磁盘仅 X 段，不足所选 CLIPn 所需）：自动从 CLIPm 补渲染建立完整链，随后从所选 CLIP 连续生成到结束`。
- 不再报错、不再静默：用户明确看到"为何从 CLIPm 开始 + 自动补全"。

### 行为矩阵（单选 CLIP4 为例，共 6 CLIP）
| 磁盘缓存 | 行为 |
|---|---|
| 0 段（空） | 自动从 CLIP1 补渲染 → CLIP4-6 连续生成（提示） |
| 2 段 | 自动从 CLIP3 补渲染 → CLIP4-6 连续生成（提示） |
| ≥3 段（完整） | 严格从 CLIP4 开始 → CLIP4-6 连续生成（不补） |

### 建议
全量渲染一次建立完整缓存后，单独选择任意 CLIP 都严格从该 CLIP 开始，零补渲染。
